// ../browser-agent/opencli/clis/notebooklm/summary.js
...
async function readNotebooklmSummaryFromPage(page) {
...
async function getNotebooklmSummaryViaRpc(page) {
  const state = await getNotebooklmPageState(page);
  if (state.kind !== "notebook" || !state.notebookId)
    return null;
  ...
  const summary = detail.filter((value, index) => index !== 0 && index !== 2 && index !== 3).find((value) => typeof value === "string" && value.trim().length >= 80);
