...
function parseMetric(value) {
  const raw = normalizeWhitespace2(value).toLowerCase();
  if (!raw)
    return 0;
  ...
  if (suffix === "k")
    return Math.round(base * 1e3);
  if (suffix === "m")
    return Math.round(base * 1e6);
  return Math.round(base);
}
function buildPostId(post) {
  const url = normalizeWhitespace2(post.url);
  if (url)
    return url;
  const author = compactRepeatedText(post.author);
  const text = normalizeWhitespace2(post.text);
...
function normalizeTimestamp(value) {
  const text = normalizeWhitespace2(value).replace(/[•·.]$/g, "").trim();
